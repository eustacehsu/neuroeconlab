...
specialSubjectall=[2 3 5 6 7 9 11 15 16 17 19]; % p-value almost 0.1
...
% modname='model_01';
% statdir = ['A:\DCJ\SPM\stat\tom\model_based\condtional\specialsubj\gainminusloss\evaluation\dur_',num2str(dur_target(t)),'\',modname];  
% mkdir(statdir)
% %dur_target
% % Duration(i(:,n))=Duration(i(:,n))%>>> has to be put inside the code
% %%
% for Subj =  specialSubjectall
% 
...
%         pmod(n).name{2} = 'Down'; 
%         pmod(n).param{2} = -Down(i(:,n));
%         pmod(n).poly{2} = 1;             %% (decision_1+decision_2+decision_3+decision_123)
%         n = n+1;
