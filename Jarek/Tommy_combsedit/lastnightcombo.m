...
statdir = ['A:\DCJ\SPM\stat\tom\lastnight\model_based\condtional\specialsubj\2groups\outcome\changenext_giusto2\dur_',num2str(dur_target(t)),'\',modname];  
mkdir(statdir)
%dur_target
...
%%                
        ...
        pmod(n).param{1} = allBet(i(:,n)); %where invest(i) comes from the previous block
        pmod(n).poly{1} = 1;       
...
% + Residual;ev2 +Residualev3 + Residualoutc3 + Residualoutc123
modname='mincomplexity';
statdir = ['A:\DCJ\SPM\stat\tom\lastnight\model_based\condtional\specialsubj\2groups\outcome\taken\dur_',num2str(dur_target(t)),'\',modname];  
mkdir(statdir)
%dur_target
% Duration(i(:,n))=Duration(i(:,n))%>>> has to be put inside the code
%%
