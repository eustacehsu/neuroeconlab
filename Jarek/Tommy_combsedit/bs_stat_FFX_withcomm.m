...
%--------------------------------------------------------------------------
for cSubj = Subj
    dSPM  = fullfile(statdir, sprintf('Subject%02d', cSubj), 'SPM'); % T_T
    dLog  = fullfile(statdir, sprintf('Subject%02d', cSubj), 'LOG'); % defines the GLMs model
    dRun1 = fullfile(datadir, sprintf('Subject%02d', cSubj), 'Functional', 'Run1'); %data from run 1
    dRun2 = fullfile(datadir, sprintf('Subject%02d', cSubj), 'Functional', 'Run2'); %data from run 2
    dRun3 = fullfile(datadir, sprintf('Subject%02d', cSubj), 'Functional', 'Run3'); %data from run 3
    ...
    matlabbatch{1}.spm.stats.fmri_spec.sess(1).multi_reg{1} = fRP1; %adds movement parameters to the regression
